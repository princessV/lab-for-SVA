...
WebBench is written totally by C. There is a Makefile in the top level of the directory. So we just have to run the following linux command to compile the source code.
...
\noindent
From the Makefile, the compiler is \textit{gcc}. Our operating system is Ubuntu 16.04

\subsection{Dump the headers}
We run the following command to dump the headers.
\begin{lstlisting}
$ objdump -h webbench
\end{lstlisting}
...
\subsection{Analyze the library dependencies}
We run the following command to print the shared library dependencies.
\begin{lstlisting}
$ ldd webbench
\end{lstlisting}
We get these library dependencies.
...
\noindent
linux-gate.so.1 is the Linux Virtual Dynamic Shared Object. This file only exists in each executables address space. \\
libc.so.6 is libc shared object version 6, which sits in /lib/i386-linux-gnu and its memory location is 0x00007ffd99184000. \\
The last line talks about ld-linux.so version 2 under /lib. This fellow will find and load shared libraries node needs. It will prep those libraries and run them.
...
\subsection{Identify \textit{main()} function vulnerabilities}
...
\subsection{Write Python script}
We write a Python script to read the ELF file and print some information out. The following is the script.
{
\scriptsize
\begin{lstlisting}
def parser():
